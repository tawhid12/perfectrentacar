@php
$offers = \DB::table('coupons')->get();
@endphp
@forelse($offers as $offer)
<div class="jumbotron">
    <div class="container">
        <h1>{{$offer->code}}</h1>
        <p>Apply This Coupon code to get discount</p>
        <p>{{$offer->value}} % OFF</p>
...
<div class="thumbnail no-border no-padding thumbnail-car-card clearfix ">
    ...
    <div class="caption">
        ...
        <h4 class="caption-title"><a href="{{route('products.show',['products'=> $product->alias ])}}">{{$product->title ?? ''}}</a>
        </h4>
        ...
        <table class="table">
            <tbody>
                <tr>
                    ...
                    <td class="buttons"><a class="btn btn-theme" href="{{route('products.show',['products'=> $product->alias ])}}">
...
<div class="pagination-wrapper">
    <ul class="pagination">
        ...
        @for($i = 1; $i <= $products->lastPage(); $i++)
            ...
            <li><a href="{{ $products->url(($products->currentPage() + 1)) }}">{{__('Next')}} <i class="fa fa-angle-double-right"></i></a></li>
            @endif
